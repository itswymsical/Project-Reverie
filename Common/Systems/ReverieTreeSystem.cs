...
using ReverieMod.Content.Tiles.MyceliumGrotto;
    ...
    {    
        public class ReveriePass : GenPass
        {
            ...
            protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
            {
                ...
                int trunkBottomY = (int)(Main.rockLayer + (Main.maxTilesY - Main.rockLayer) / 9);
                int trunkWidth = 18;
                ...
                for (int y = trunkTopY; y <= trunkBottomY; y++)
                {
                    ...
                    for (int x = leftBound; x <= rightBound; x++)
                    {
                        ...
                        WorldGen.PlaceTile(x, y, TileID.LivingWood, true, forced: true);
                    }
                }
                for (int y2 = trunkTopY; y2 <= trunkBottomY; y2++)
                {
                    ...
                    for (int x2 = leftBound; x2 <= rightBound; x2++)
                    {
                        ...
                        WorldGen.PlaceWall(x2, y2, WallID.LivingWoodUnsafe, true);
                    }
                }
                ...
                for (int x = centerX - horizontalRadius; x <= centerX + horizontalRadius; x++)
                {
                    for (int y = centerY - verticalRadius; y <= centerY + verticalRadius; y++)
                    {
                        if (WorldGenHelpers.IsPointInsideEllipse(x, y, centerX, centerY, horizontalRadius, verticalRadius))
                        {
                            ...
                            WorldGen.PlaceWall(x, y, WallID.LivingLeaf, true);
                            WorldGen.PlaceTile(x, y, ModContent.TileType<LoamTile>(), mute: true, forced: true);
                        ...
                        else if (WorldGenHelpers.IsPointNearOvalEdge(x, y, centerX, centerY, horizontalRadius, verticalRadius))
                        {
                            if (Main.rand.NextFloat() < 0.8f)
                            {
                                ...
                                for (int i = 0; i < branchLength; i++)
                                {
                                    ...
                                    {
                                        WorldGen.PlaceTile(branchX, branchY, ModContent.TileType<LoamTile>(), mute: true, forced: true);
                                    }
                                }
                            }
                        }
                ...
                for (int x = centerX - horizontalRadius; x <= centerX + horizontalRadius; x++)
                {
                    for (int y = centerY - verticalRadius; y <= centerY + verticalRadius; y++)
                    {
                        if (WorldGenHelpers.IsPointInsideEllipse(x, y, centerX, centerY, horizontalRadius, verticalRadius))
                        {
                            ...
                            {
                                WorldGen.digTunnel(x, y, Main.rand.Next(1, 7), Main.rand.Next(1, 3), Main.rand.Next(1, 5), Main.rand.Next(3, 7));
                            }
