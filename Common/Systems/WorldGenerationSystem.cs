...
using static ReverieMod.Common.Systems.LigneousTempleSystem;
...
namespace ReverieMod.Common.Systems
{
    public class WorldGenerationSystem : ModSystem
    {
        ...
        {
            int ReverieIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Settle Liquids"));
            if (ReverieIndex != 1)
            {
            ...
            int LigneousTempleIndex = tasks.FindIndex(genpass => genpass.Name.Equals("Stalac"));
            if (LigneousTempleIndex != 1)
            {
                tasks.Insert(LigneousTempleIndex + 1, new LigneousTemplePass("Ligneous Temple", 74f));
            }
        }
    }
}